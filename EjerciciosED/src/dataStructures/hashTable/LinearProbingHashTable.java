/**
 * ALUMNO: Ángel Manuel Joaquín Yébenes
 * GRUPO: 2 C Ingeniería Informática
 *
 ...
 */public class LinearProbingHashTable<K,V> implements HashTable<K,V>{
	...
	/** EJERCICIO A COMPLETAR **/
	public V search(K key) { int idx = searchIdx(key); return values[idx];
	}
	...
	/** EJERCICIO A COMPLETAR **/
	public boolean isElem(K key) { return search(key)!=null ? true : false ; }
		...
		public void advance() {
			if(hasNext()) {
				do{
					...
					visited++;
				}while(hasNext() && keys[nextIdx]==null);
			}else{
				throw new NoSuchElementException("No quedan elementos por visitar");
			}
		}
